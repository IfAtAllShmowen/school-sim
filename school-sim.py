...
def move_camera(move_y, move_x):
	global camera_y
	global camera_x
	new_y = camera_y + move_y
	new_x = camera_x + move_x
	if move_y < 0:
...
def make_border_wall():
	for a in range(map_y):
		make_object("border", wall_icon, a, 0)
		make_object("border", wall_icon, a, map_x - 1)
	for a in range(map_x):
		make_object("border", wall_icon, 0, a)
		make_object("border", wall_icon, map_y - 1, a)
...
def game_startup():
	...
	make_screen()
	make_border_wall()
	make_object("player", player_icon, 4, 4)
	make_rand_objects(100, "wall", wall_icon, 0, 0, map_y, map_x)
	make_rand_objects(50, "door", door_icon, 0, 0, map_y, map_x, ["filler", "wall"])
